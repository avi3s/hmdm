...
awk !/photo/ plugins\pom.xml | awk !/licensing/ | awk !/contacts/ | awk !/deviceimport/ | awk !/deviceexport/ | awk !/devicereset/ | awk !/devicelocations/ | awk !/knox/ | awk !/openvpn/ > %TARGET_DIR%\plugins\pom.xml
...
echo Preparing DEVICE INFO PLUGIN module...
mkdir %TARGET_DIR%\plugins\deviceinfo

cp --parents -v plugins\deviceinfo\pom.xml %TARGET_DIR%
cp --parents -vr plugins\deviceinfo\src\ %TARGET_DIR%

echo Preparing PUSH PLUGIN module...
mkdir %TARGET_DIR%\plugins\push

cp --parents -v plugins\push\pom.xml %TARGET_DIR%
cp --parents -vr plugins\push\src\ %TARGET_DIR%

echo Preparing SERVER module...
mkdir %TARGET_DIR%\server
mkdir %TARGET_DIR%\server\webtarget

awk !/photo/ server\pom.xml | awk !/licensing/ | awk !/contacts/ | awk !/deviceimport/ | awk !/deviceexport/ | awk !/devicereset/ | awk !/devicelocations/ | awk !/knox/ | awk !/openvpn/ > %TARGET_DIR%\server\pom.xml

cp --parents -vr server\src\ %TARGET_DIR%
cp --parents -vr server\conf\ %TARGET_DIR%

awk !/license/ server\conf\context.xml | awk !/rebranding/ > %TARGET_DIR%\server\conf\context.xml
...
awk !/photo/ server\build.properties | awk !/licensing/ | awk !/license/ | awk !/deviceimport/ | awk !/deviceexport/ | awk !/devicereset/ | awk !/devicelocations/ | awk !/knox/ | awk !/openvpn/ | awk !/rebranding/ > %TARGET_DIR%\server\build.properties
...
rem echo Preparing INSTALLATOR ...
rem mkdir %TARGET_DIR%\install
rem mkdir %TARGET_DIR%\install\sql

rem cp --parents -v BUILD.txt %TARGET_DIR%
rem cp --parents -v hmdm_install.sh %TARGET_DIR%
rem cp --parents -v LICENSE %TARGET_DIR%
rem cp --parents -v README.md %TARGET_DIR%
rem cp --parents -v TROUBLESHOOTING.txt %TARGET_DIR%
rem cp --parents -v NOTICE %TARGET_DIR%
rem cp --parents -vr install %TARGET_DIR%
